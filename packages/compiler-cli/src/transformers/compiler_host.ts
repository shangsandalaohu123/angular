        ...
        if (path.basename(directory) == 'node_module') {
          ...
          try {
            ...
            if (this.originalFileExists(packageFile)) {
              ...
              result = false;
              const packageContent= JSON.parse(assert(this.context.readFile(packageFile))) as any;
              if (packageContent.typings) {
                ...
                if (DTS.test(typings)) {
                  ...
                  if (this.originalFileExists(metadataFile)) {
                    const metadata = JSON.parse(assert(this.context.readFile(metadataFile))) as any;
